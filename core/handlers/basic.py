...
async def get_PUINFO(message: Message, bot: Bot):
    photo = FSInputFile(path=r'C:\Users\Bogdan\Desktop\i.webp')
    await bot.send_photo(message.chat.id, photo=photo, caption='Удобное расположение Общежитие №9 расположено на одной из главных улиц города Казани — ул. Пушкина,32. Это не помеха для твоего покоя и тишины, потому что дом находится в некотором отдалении от шумной магистрали. \n'
...
async def get_KPIINFO(message: Message, bot: Bot):
    photo = FSInputFile(path=r'C:\Users\Bogdan\Desktop\i.webp')
    await bot.send_photo(message.chat.id, photo=photo, caption='Если ты приехал учиться в Казанский университет из другого города, заботливые родители могут снять для тебя квартиру. Но куда интереснее и дешевле жить в студенческом городке, в общежитии №4. \n'
